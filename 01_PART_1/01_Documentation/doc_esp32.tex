...
\begin{document}
	...
	Innerhalb der \texttt{set\_motion\_led(motion)} Funktion, setzen wir nun den LED-Status basierend auf dem booleschen Wert, der in dem Funktionskopf mitgegeben wird. Sollte dieser \texttt{True} sein, leuchtet die LED.
	...
	\end{lstlisting}
	Wir deklarieren uns zuerst 3 Konstanten, die wir in der späteren Rechnung wiederkehrend benötigen.
	\texttt{gamma} Variable beschreibt, um wie viel sich der Widerstand bei jeder Einheit der Lichtänderung ändert. \texttt{rl10} Variable beschreibt die Menge an Widerstand von dem Fotowiderstand \textit{LM393} in kΩ. 
	\texttt{ref\_voltage} ist die Variable für die Spannung ausgehend von dem \textit{ESP32}.
	\newline
	\newline
	Um nun den Wert für die Spannung \texttt{voltage} zu berechnen, muss zuerst die \textbf{12-Bit} Ausgabe von dem Fotowiderstand in eine \textbf{10-Bit} umgewandelt werden, dies geschieht anhand von \texttt{/ 4}.
	
	Im Anschluss konvertieren wir die Zahl mithilfe von \texttt{/ 1024 * ref\_voltage} in die Spannung, weil wir eine Anschlussspannung von \textbf{3,3V} haben.
	
	In Zeile 8 kalkulieren wir den Widerstand \texttt{resistance}, indem wir einen 2kΩ Widerstand mal unsere aktuelle Spannung rechnen. Das Resultat wird im Anschluss durch das Ergebnis von \texttt{1 - voltage} durch \texttt{ref\_voltage} gerechnet.
	...
	\begin{enumerate}
		\item (\texttt{rl10 * 1e0}) konvertiert die 50kΩ in 50,000Ω.
		\item  \texttt{pow(10, gamma) / resistance} Teil der konstanten Skalierung für den 10-Lux-Referenzpunkt.
		\item \texttt{pow(...,1/gamma)} Die Potenzfunktion, die verwendet wird, um den Lux-Wert aus dem Widerstandsverhältnis zu berechnen.
	\end{enumerate}
	...
	\end{lstlisting}
	In dieser Zeile definieren wir eine Komponente mit mehreren Pin Anschlüssen. Dabei definieren wir zuerst die Breie \& Höhe des Bildschirms, im Anschluss deklarieren wir die Pins \texttt{sda} \& \texttt{scl}, damit wir Daten zwischen dem \textit{ESP32} \& dem \textit{SSD1306} senden können. 
	...
	def display_text(lineOne,lineTwo,lineThree,lineFour):
		...
		display.show()
	\end{lstlisting}
	Damit sich der Text, bei einem Update \textbf{nicht} überlappt, muss zuvor die Fläche \textbf{zurückgesetzt} werden. Das Zurücksetzen der Anzeigefläche passiert, indem wir eine schwarze Fläche \texttt{display.fill(0)} zeichnen. Im Anschluss schreiben wir die Information, welche wir aus dem Funktionskopf erhalten, auf den LCD-Monitor und aktualisieren die Anzeige mit \texttt{display.show()}.
	\newpage
	...
	while True:
		...
		sleep(1)
	\end{lstlisting}
	Die Funktion \texttt{handle\_lcd\_data\_display(temperature,humidity, motion, luminance)} dient als Wrapper-Funktion zwischen der Datenübergabe in der While-Schleife \& umsetzenden Funktion \texttt{display\_text(...)}
	\newline
	\newline
	...
	\chapter{Aufgabe 4}
	\section{Anforderungen}
	Ergänzen Sie Ihren Prototypen dahingehend, dass eine grüne, gelbe oder rote LED leuchten soll wenn die Temperatur >= 30 °C (rot), >= 25 °C und < 30 °C (gelb) oder <= 20 °C (grün) beträgt.
	...
	\begin{table}[h!]
		\centering
		\caption{Verteilung der Pin-Nutzung bei dem ESP32}
		\label{tab:teil_4_steckbrett_esp32}
		\begin{tabular}{|l|l|} 
			\hline
			Pin & Aufgabe \\
			\hline
	...
	Die Funktion \texttt{set\_temperature\_lights(red\_on, orange\_on, green\_on)} setzt den Status für jede einzelne LED.
	...
	\end{lstlisting}
	Nun fügen wir die Logik mit der \texttt{handle\_temperature\_led\_lights} ein. Sollte die Temperatur über 25°C liegen, geht die rote LED an. Liegt die Temperatur zwischen 25°C \& 20°C, geht die gelbe LED an. Sollten keiner dieser Zustände zutreffen, geht die grüne LED an.
	\newpage
	\begin{lstlisting}
	while True:
		temp, humid = get_device_temperature_and_humidity()
	...
	Innerhalb der While-True Schleife können wir nun die Funktion \\ \texttt{handle\_temperature\_led\_lights(temp)} aufrufen \& die Temperatur übergeben.
